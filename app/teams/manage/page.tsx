...
const AVAILABLE_ROLES: Record<string, string[]> = {
  'captain': ['vice_captain', 'leader', 'group_leader', 'elite', 'member', 'member'],
  'vice_captain': [],
  'leader': [],
  'group_leader': [],
  'elite': [],
...
const KICK_PERMISSIONS: Record<string, string[]> = {
  'captain': ['vice_captain', 'leader', 'group_leader', 'elite', 'member', 'member'],
  'vice_captain': ['leader', 'group_leader', 'elite', 'member', 'member'],
  'leader': [],
  'group_leader': [],
  'elite': [],
  'member': [],
...
const getRoleColor = (role: string) => {
  switch (role) {
    case 'captain': return 'bg-gradient-to-r from-yellow-400 to-orange-500 text-white'
    case 'vice_captain': return 'bg-gradient-to-r from-purple-400 to-purple-600 text-white'
    case 'leader': return 'bg-gradient-to-r from-blue-400 to-blue-600 text-white'
    case 'group_leader': return 'bg-gradient-to-r from-orange-400 to-orange-600 text-white'
    case 'elite': return 'bg-gradient-to-r from-green-400 to-green-600 text-white'
...
export default function TeamManagePage() {
  ...
  const fetchTeamData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const captainTeam = (memberData as Array<{ role: string; team_id: string }>).find(member => member.role === 'captain' || member.role === '队长')
      const targetTeam = captainTeam || (memberData as Array<{ role: string; team_id: string }>)[0]
      console.log('Fetched user role:', targetTeam.role)
      // 统一角色值为英文
      const normalizedRole = targetTeam.role === '队长' ? 'captain' : 
                            targetTeam.role === '副队长' ? 'vice_captain' : 
                            targetTeam.role === '领队' ? 'leader' : 
                            targetTeam.role === '组长' ? 'group_leader' : 
                            targetTeam.role === '队员' ? 'member' : targetTeam.role
  ...
  const handleDrop = async (e: React.DragEvent, targetMember: Member) => {
    e.preventDefault()
    if (!draggedMember || draggedMember.id === targetMember.id || userRole !== 'captain') return
  ...
  const handleKick = () => {
    ...
    if (selectedMember.role === 'captain') {
      alert('队长不能被移出战队')
      return
    }
    setShowActionModal(false)
                ...
                {members.map((member) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group card card-hover border-2 transition-all duration-200 ${
                  ...
                  <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
                    <div className="text-center mb-6">
                      ...
                      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-lg text-sm font-medium mt-2 ${getRoleColor(selectedMember.role)}`}>
                        ...
                        <span>{ROLE_NAMES[selectedMember.role] || selectedMember.role}</span>
                      </span>
                    </div>
                    ...
                    <div className="space-y-2.5">
                      ...
                      {selectedMember && KICK_PERMISSIONS[userRole]?.includes(selectedMember.role) && selectedMember.role !== 'captain' && (
                        <button
                          onClick={handleKick}
                        ...
                        >
                          <span className="text-xl">🚫</span>
                          <div>
                            <div className="font-medium text-red-600 text-sm">移出战队</div>
                  ...
                  <div className="bg-white rounded-t-3xl sm:rounded-2xl p-6 w-full max-w-sm shadow-xl animate-slide-up">
                    ...
                    <div className="space-y-2.5">
                      ...
                      {selectedMember && KICK_PERMISSIONS[userRole]?.includes(selectedMember.role) && selectedMember.role !== 'captain' && (
                        ...
                        >
                          <span className="text-2xl">🚫</span>
                          <span className="font-medium text-red-600">移出战队</span>
                        </button>
                      )}
              ...
              {showKickConfirm && selectedMember && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-xl">
                    ...
                    <h3 className="text-lg font-bold text-gray-800 mb-2">确认移出战队？</h3>
