...
export default function ApplicationsPage() {
  ...
  const getTeamApplications = useCallback(async () => {
    ...
    try {
      ...
      // 统一角色值为英文
      const normalizedRole = teamMember.role === '队长' ? 'captain' : 
                            teamMember.role === '副队长' ? 'vice_captain' : 
                            teamMember.role === '领队' ? 'leader' : 
                            teamMember.role === '组长' ? 'group_leader' : 
                            teamMember.role === '队员' ? 'member' : teamMember.role

      // 检查是否是队长或副队长
      if (normalizedRole !== 'captain' && normalizedRole !== 'vice_captain') {
        setError('您没有权限查看申请')
        setLoading(false)
        return
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <PageLayout>
        <div className="container mx-auto px-4 pb-8">
          ...
          {error && (
            <div className="mb-4 p-3 bg-red-100 text-red-600 rounded-lg card">
              {error}
            </div>
          )}

          {success && (
            <div className="mb-4 p-3 bg-green-100 text-green-600 rounded-lg card">
              {success}
            </div>
          )}
            ...
            {applications.length === 0 ? (
              ...
              <div className="space-y-4">
                {applications.map((app) => (
                  <div key={app.id} className="border border-gray-300 rounded-lg p-4 card card-hover">
                    <div className="flex items-center gap-4 mb-4">
