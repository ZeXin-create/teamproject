...
-- 2. 为 team_sales 表添加 team_id 列
ALTER TABLE team_sales ADD COLUMN team_id UUID REFERENCES teams(id);
...
-- 4. 补全 player_profiles 表的 RLS 策略
-- 所有成员可读自己的资料
...
-- 6. 补全 team_sales 表的 RLS 策略（包含队长管理策略）
-- 所有人可读
...
-- 7. 更新 team_members 表的 RLS 策略，添加副队长权限
-- 副队长可读所有成员记录
...
-- 为 team_members 表添加更新策略，只允许修改非角色字段
